...
// Schema-breaking changes are allowed; keep a fresh table name.
const CAPSULES_TABLE: &str = "capsules_v2";
...
// ML-backed emotion classifier (ONNX) is defined below.

fn extract_user_and_assistant_text(slice: &str) -> (String, String) {
...
fn map_go_emotions(label: &str, score: f32) -> EmotionMeta {
...
impl EmotionModel {
    async fn load(cfg: EmotionConfig) -> anyhow::Result<Self> {
        ...
        let id2label = parse_id2label(&config_json)
            .unwrap_or_else(|| vec!["neutral".to_string()]);
    ...
    fn classify_one(&mut self, text: &str) -> anyhow::Result<(String, f32)> {
        let enc = self
            .tokenizer
            .encode(text, true)
...
async fn insert_capsule_row(
    ...
    let assistant_emotion_arr = Arc::new(StringArray::from(vec![assistant_emotion.map(|e| e.label.as_str())]));
    let assistant_emotion_conf_arr = Arc::new(Float32Array::from(vec![assistant_emotion.map(|e| e.confidence)]));
    let assistant_valence_arr = Arc::new(Float32Array::from(vec![assistant_emotion.map(|e| e.valence)]));
    let assistant_intensity_arr = Arc::new(Float32Array::from(vec![assistant_emotion.map(|e| e.intensity)]));
...
async fn run_init(
    ...
    for (capsule, meta) in capsules {
        ...
        insert_capsule_row(&db, &embedder, 0, seq, now_ms, &meta, None, None, &capsule).await?;
        seq += 1;
    }
...
impl ServeState {
    fn new(embedder: Embedder, emotion: EmotionModel) -> Self {
        Self {
            embedder,
...
async fn run_serve(bind: SocketAddr, embedder: Embedder) -> anyhow::Result<()> {
    ...
    let emotion = EmotionModel::load(EmotionConfig::default()).await?;
    let state = ServeState::new(embedder, emotion);
