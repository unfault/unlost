...
- **`unlost reflect`**: New command that reads the per-turn `TurnEval` timeline for a session or time window and generates a structured coaching/diagnostics narrative via LLM. Three modes: `--mode coach` (developer collaboration habits), `--mode diagnose` (agent drift and failure patterns), and `--mode both`. Supports `--session <id>` and `--since <duration>` scoping. Aggregated session statistics (avg clarity, freshness, intensity, outcome distribution, top flags) are included in the LLM context alongside the per-turn timeline — the LLM sees only structured telemetry, no raw transcript.
