...
### Added
- **`TurnEval`**: Per-turn evaluation metadata computed on-the-fly at flush time with zero LLM calls. Each capsule now carries 12 agent-tuning (`tune`) dimensions — persisted governor `SymptomChannels` previously discarded after friction decisions — plus 5 developer coaching (`coach`) dimensions: `clarity` (request specificity), `context_freshness` (cache ratio + frustration slope, captures compaction signal), `verification_rigor` (tool outcome presence), `decision_progress` (decision/symbol diff from prior turn), and `scope_discipline` (symbol spread + category diversity). Behavioral flags (`session_heavy`, `session_too_long`, `retry_loop`, `blind_acceptance`, etc.) are derived from thresholds on both dimensions. Stored in LanceDB (`te_*` columns with schema evolution), JSONL capsule log, and metrics events. Displayed in `unlost inspect`. Designed to power `unlost reflect` in a subsequent release — a reflect-time LLM can reconstruct session quality from structured telemetry only, with no raw transcript required.
- **`unlost reflect`**: New command that reads the per-turn `TurnEval` timeline for a session or time window and generates a structured coaching/diagnostics narrative via LLM. Three modes: `--mode coach` (developer collaboration habits), `--mode tune` (agent drift and failure patterns), and `--mode both`. Supports `--session <id>` and `--since <duration>` scoping. Aggregated session statistics (avg clarity, freshness, intensity, outcome distribution, top flags) are included in the LLM context alongside the per-turn timeline — the LLM sees only structured telemetry, no raw transcript. Output uses a dedicated ANSI renderer with coloured section headers, score colouring (green/yellow/red by value), confidence markers, and `turn N` reference highlighting.
- **Outcome backfill**: At each checkpoint, `te_outcome_hint` is retroactively computed for all conversational capsules in the session using a deterministic lookahead heuristic (decision text overlap, symbol set diff, failure mode, and friction flags). Values: `progressed`, `stalled`, `regressed`, `unclear`. Written back to LanceDB via `UPDATE` — existing `unlost reflect` runs on the same session will see corrected outcomes.
- **`TurnEval` in recall/query results**: All three `CapsuleHit` retrieval paths (`query_capsules_lancedb`, `scan_capsules_lancedb`, and the fan-out path) now read and populate `turn_eval` from LanceDB. A shared `read_turn_eval` helper eliminates the previous duplicate inline logic.
- **`TurnEval` backfill on reindex**: `unlost reindex` now automatically populates `TurnEval` for every capsule in history without any extra flag. Post-v0.13 capsules have full data restored from the JSONL `turn_eval` field. Pre-v0.13 capsules get coach dimensions computed from capsule content and a rolling 8-turn history window; tune channels (governor EMA state) are set to 0 with a `v1-reindex` version marker since that state is not recoverable from JSONL alone.

### Changed
- **`unlost reflect --mode diagnose` → `--mode tune`**: The agent-facing persona is now called `tune` (`--mode tune`) throughout — CLI, prompts, inspect output, and comments — to make it clear the target is agent behaviour improvement, not problem diagnosis. `coach` and `both` are unchanged.
