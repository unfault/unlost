...
- **`unlost export`**: New command that exports capsules as structured markdown to a user-selected directory, enabling a "second brain" workflow consumable by Obsidian, Logseq, Hugo, or any markdown-aware harness. Capsules are organised into a fixed 15-bucket taxonomy (`architecture`, `debugging`, `devops`, `documentation`, `feature`, `meta`, `notes`, `performance`, `planning`, `refactoring`, `release`, `review`, `testing`, `ux`, `other`). When an LLM is configured, all raw categories are batch-classified in a single call; otherwise fast keyword heuristics apply. Each category folder gets a `README.md` table of its capsules; the root `INDEX.md` links all categories. Individual files include YAML front-matter (`id`, `date`, `category`, `symbols`, `failure_mode`, `tags`) for Obsidian dataview queries and static-site frontends. Export is incremental by default; `--force` regenerates all; `--narrative` triggers LLM-written prose summaries per category README.
...
- **`unlost_mcp` OpenCode skill** (`.opencode/skills/unlost-mcp/SKILL.md`): Teaches the agent when to call each MCP tool proactively — before edits, before reversals, at the start of complex subtasks, and for cross-project memory.
...
- **`unlost note`**: New command to capture manual notes into workspace memory. Accepts positional text, `--stdin` for piped input, and `--source <label>` for free-form categorization. If no project (git repo or manifest) is detected, notes land in a `global` workspace under `~/.local/share/unlost/workspaces/global/`. The `--global` flag forces this regardless of the current directory. Symbols are auto-extracted from note text for retrieval via `unlost query --symbol`. Notes are written to both LanceDB and `capsules.jsonl`, surviving `unlost reindex`. Source pointer URI scheme `note+local://<root>#<ts_ms>` renders as `manual note (2026-05-16)` in query/thread footers.
...
- **`unlost note`**: Captured note text is now word-wrapped at 80 columns before storage, so `query`, `inspect`, and `thread` display it cleanly without per-command wrapping logic.
- **Thread provenance**: Anchor cluster headers (where it landed / the turn / where it started) and timeline dates now render provenance on a separate indented line wrapped to `WRAP_WIDTH`, preventing single-line overflows with long source label lists.
