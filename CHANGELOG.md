...
- **`unlost thread`**: New command that maps when a topic was explored over time, across all registered projects. Results are sorted oldest-to-newest and rendered as a notes-style retrospective: LLM synthesis first (when enabled), then day-grouped notes with 80-column wrapping, quiet metadata, dim support lines, folded near-duplicate echoes, and dormancy markers (`N days later`) for gaps >7d. Optional LLM synthesis describes the intellectual arc — what kept recurring, how the framing shifted, where it landed — without framing any moment as "unresolved". Cross-workspace retrieval is the default. Supports `--since`, `--no-llm`, `--limit`, `--output plain`, `--llm-model`.
